...
namespace Patapon4TLB.GameModes
{
	...
	public class MpVersusHeadOnGameModeObsolete : GameModeSystem<MpVersusHeadOn>
	{
		...
		protected override void OnCreate()
		{
			...
			{
				All = new ComponentType[] {typeof(Translation), typeof(LivableHealth), typeof(VersusHeadOnUnit)}
			});
		...
		public override unsafe void OnGameModeUpdate(Entity gmEntity, ref MpVersusHeadOn gameMode)
		{
			...
			if (IsInitialization())
			{
				...
				// ----------------------------- //
				// Add players
				Entities.With(m_PlayerWithoutGameModeDataQuery).ForEach((Entity e) => { EntityManager.AddComponent(e, typeof(VersusHeadOnPlayer)); });
			...
			switch (gameMode.PlayState)
			{
						...
						for (var form = 0; form != entities.Length; form++)
						{
							...
							for (var arm = 0; arm != armies.Length; arm++)
							{
								...
								for (var unt = 0; unt != units.Length; unt++)
								{
									...
									EntityManager.AddComponentData(ent, new VersusHeadOnUnit
									{
				...
				{
					Entities.With(m_UnitQuery).ForEach((Entity e, ref Translation translation, ref VersusHeadOnUnit VersusHeadOnUnit, ref UnitDirection direction, ref LivableHealth livableHealth) =>
					{
						var spawnPosition = EntityManager.GetComponentData<LocalToWorld>(Teams[VersusHeadOnUnit.Team].SpawnPoint).Position;

						translation.Value   =  spawnPosition;
						translation.Value.x += VersusHeadOnUnit.FormationIndex * 0.75f * direction.Value;

						VersusHeadOnUnit.TickBeforeSpawn = new UTick {Value = -1};
				...
				case MpVersusHeadOn.State.Playing:
				{
					...
					using (var healthArray = m_LivableQuery.ToComponentDataArray<LivableHealth>(Allocator.TempJob))
					{
						for (var ent = 0; ent != healthArray.Length; ent++)
						{
							...
							VersusHeadOnUnit? VersusHeadOnUnit = null;
							if (EntityManager.HasComponent<VersusHeadOnUnit>(entityArray[ent]))
							{
								VersusHeadOnUnit = EntityManager.GetComponentData<VersusHeadOnUnit>(entityArray[ent]);
								team = VersusHeadOnUnit.Value.Team;
							}
							...
							if (health.ShouldBeDead() && !health.IsDead)
							{
								...
								{
									var bonus = 0;
									if (VersusHeadOnUnit != null && VersusHeadOnUnit.Value.KillStreak > 1)
									{
										bonus += (VersusHeadOnUnit.Value.KillStreak - 1) * 5;
									}
								...
								for (var story = 0; !hasInstigator && story != history.Length; story++)
								{
									if (history[story].Instigator != default && history[story].Value < 0)
									{
										...
										if (lastInstigator == default
										    && EntityManager.HasComponent<VersusHeadOnUnit>(history[story].Instigator))
										{
											lastInstigator = history[story].Instigator;

											var gmUnit = EntityManager.GetComponentData<VersusHeadOnUnit>(lastInstigator);
											gmUnit.KillStreak++;
								...
								if (VersusHeadOnUnit != null)
								{
									...
									var nonNull = VersusHeadOnUnit.Value;
					...
					using (var gmUnitArray = m_UnitQuery.ToComponentDataArray<VersusHeadOnUnit>(Allocator.TempJob))
					using (var healthArray = m_UnitQuery.ToComponentDataArray<LivableHealth>(Allocator.TempJob))
					{
						for (var ent = 0; ent != entityArray.Length; ent++)
						{
							...
							ref var VersusHeadOnUnit = ref UnsafeUtilityEx.ArrayElementAsRef<VersusHeadOnUnit>(gmUnitArray.GetUnsafePtr(), ent);
							ref var health = ref UnsafeUtilityEx.ArrayElementAsRef<LivableHealth>(healthArray.GetUnsafePtr(), ent);

							// If the unit should respawn, then respawn him
							if (health.IsDead && VersusHeadOnUnit.TickBeforeSpawn <= ServerTick)
							{
								health.IsDead = false;
							...
							var oppositeTeam   = Teams[1 - VersusHeadOnUnit.Team];
							var oppositeFlagTr = EntityManager.GetComponentData<Translation>(oppositeTeam.Flag).Value;
							var side           = VersusHeadOnUnit.Team == 0 ? -1 : 1;
							if ((oppositeFlagTr.x - translation.Value.x) * side >= 0)
							{
								Debug.Log($"{VersusHeadOnUnit.Team} captured the flag!");
								WinningTeam = VersusHeadOnUnit.Team + 1;
							}
						}
