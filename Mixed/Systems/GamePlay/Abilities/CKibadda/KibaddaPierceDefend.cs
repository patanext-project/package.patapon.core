...
{
	public struct KibaddaPierceDefend : IComponentData
	{
		...
		public class Provider : BaseRhythmAbilityProvider<KibaddaPierceDefend>
		{
			public override    string MasterServerId   => nameof(P4OfficialAbilities.KibaPierceDefend);
			...
			public override void SetEntityData(Entity entity, CreateAbility data)
			{
				...
				EntityManager.SetComponentData(entity, new KibaddaPierceDefend {AttackHitBox      = hitbox});
		...
		public class System : BaseAbilitySystem
		{
			protected override void OnUpdate()
			{
				...
				Entities.ForEach((ref KibaddaPierceDefend ability, ref AbilityControlVelocity control, in AbilityState state, in AbilityEngineSet engineSet, in Owner owner) =>
				{
					...
					var seekRange = impl.UnitPlayState[owner.Target].AttackSeekRange * 0.7f;
					var rushToTarget = ability.AttackTick > 0 || seekState.Enemy != default && state.IsActive;
					if (rushToTarget && seekState.Enemy != default && seekState.Distance < seekRange)
					{
						control.IsActive       = true;
						control.Acceleration   = 17.5f;
						control.TargetPosition = direction.Value * seekRange;
					}
					else if (state.IsActiveOrChaining)
					{
